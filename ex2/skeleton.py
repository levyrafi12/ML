...
class Assignment2(object):
	...
	def draw_sample_intervals(self, m, k):
		...
		for inter in intervals:
			plt.hlines(0.8, inter[0], inter[1], 'b', lw=3) # print horizontal line
	...
	def experiment_m_range_erm(self, m_first, m_last, step, k, T):
		...
		plt.text(0.5, 0.9, 'red = Es', transform=plt.gca().transAxes, ha='center')
		plt.text(0.5, 0.85, 'blue = Ep', transform=plt.gca().transAxes, ha='center')
		title = 'm_range_erm'
	...
	def experiment_k_range_erm(self, m, k_first, k_last, step):
		...
		plt.axis([k_first, k_last, E.min(), E.max()])
		plt.text(0.5, 0.9, 'red = Es', transform=plt.gca().transAxes, ha='center')
		plt.text(0.5, 0.85, 'blue = Ep', transform=plt.gca().transAxes, ha='center')
		title = 'k_range_erm'
		plt.xlabel('k')
	...
	def experiment_k_range_srm(self, m, k_first, k_last, step):
		...
		"""
		num_k = int((k_last - k_first) / step + 1)
		E = np.zeros((num_k, 4), dtype=float)
		...
		plt.axis([k_first, k_last, E.min(), E.max()])
		plt.text(0.8, 0.6, 'red = Es', transform=plt.gca().transAxes, ha='center')
		plt.text(0.8, 0.55, 'blue = Ep', transform=plt.gca().transAxes, ha='center')
		plt.text(0.8, 0.5, 'green = PenaltySrm', transform=plt.gca().transAxes, ha='center')
		plt.text(0.8, 0.45, 'yellow = Es + PenaltySrm', transform=plt.gca().transAxes, ha='center')
		plt.xlabel('num intervals (K)')
		plt.ylabel('Error')
		title = 'k_range_srm'
		plt.title(title)
		plt.savefig(title + '.png')
	...
	def srm_penalty(self, k, m, delta):
		...
		# vcdim(H_k) = 2k
		return np.sqrt((1 / m) * (2 * k * np.log(m / (2 * k)) + np.log(1 / delta)))
